...
bool NcanGo(gf::Vector2i from, char direction, const std::vector<gf::Vector2i> &usefulVertices, gf::Vector2i &where)
{
    int deltaX = 0;
    int deltaY = 0;
    bool isHorizontal; // true -> horizontal, false -> vertical
...
void Wall::setSortedVertices(gf::Vector2i from, std::vector<gf::Vector2i> usefulVertices, std::vector<gf::Vector2i> occupiedCells)
{
    ...
    do
    {
        bool canGo = NcanGo(currentVertex, directions[currentDirection], usefulVertices, nextVertex);
        bool canGo2 = NcanGo(currentVertex, directions[(currentDirection + 2) % 4], usefulVertices, nextVertex2);
